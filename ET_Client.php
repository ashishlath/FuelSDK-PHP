...
class ET_Client extends SoapClient
{
    ...
    private $wsdlLoc, $debugSOAP, $lastHTTPCode, $clientId, $clientSecret, $appsignature, $endpoint, $tenantTokens, $tenantKey, $xmlLoc, $proxyHost, $proxyPort, $proxyLogin, $proxyPassword, $proxy;
    ...
    function __construct($getWSDL = false, $debug = false, $params = null)
    {
        ...
        if ($config) {
            ...
            if ($params && array_key_exists('proxyhost', $params)) {
                $this->proxyHost = $params['proxyhost'];
            }
            if ($params && array_key_exists('proxyport', $params)) {
                $this->proxyPort = $params['proxyport'];
            }
            if ($params && array_key_exists('proxyhost', $params) && array_key_exists('proxyport', $params)) {
                $this->proxy = $params['proxyhost'] . ":" . $params['proxyport'];
            }
            if ($params && array_key_exists('proxylogin', $params)) {
                $this->proxyLogin = $params['proxylogin'];
            }
            if ($params && array_key_exists('proxypassword', $params)) {
        ...
        try {
            ...
            $endpointResponse = restGet($url, $this->proxy);
    ...
    function refreshToken($forceRefresh = false)
    {
        if (property_exists($this, "sdl") && $this->sdl == 0) {
            parent::__construct($this->xmlLoc, array(
                'trace' => 1,
                'exceptions' => 0
            ));
        }
        try {
            ...
            if (is_null($this->getAuthTokenExpiration($this->tenantKey))) {
                ...
                $timeDiff = $currentTime->diff($this->getAuthTokenExpiration($this->tenantKey))
                    ->format('%i');
                $timeDiff = $timeDiff + (60 * $currentTime->diff($this->getAuthTokenExpiration($this->tenantKey))
                    ->format('%H'));
            }
            
            if (is_null($this->getAuthToken($this->tenantKey)) || ($timeDiff < 5) || $forceRefresh) {
                $url = $this->tenantKey == null ? "https://auth.exacttargetapis.com/v1/requestToken?legacy=1" : "https://www.exacttargetapis.com/provisioning/v1/tenants/{$this->tenantKey}/requestToken?legacy=1";
                ...
                $authResponse = restPost($url, json_encode($jsonRequest), $this->proxy);
    ...
    public function getAuthToken($tenantKey = null)
    {
        ...
        return isset($this->tenantTokens[$tenantKey]['authToken']) ? $this->tenantTokens[$tenantKey]['authToken'] : null;
    }
    ...
    function getAuthTokenExpiration($tenantKey)
    {
        ...
        return isset($this->tenantTokens[$tenantKey]['authTokenExpiration']) ? $this->tenantTokens[$tenantKey]['authTokenExpiration'] : null;
    }
    ...
    function getInternalAuthToken($tenantKey)
    {
        ...
        return isset($this->tenantTokens[$tenantKey]['internalAuthToken']) ? $this->tenantTokens[$tenantKey]['internalAuthToken'] : null;
    }
    ...
    function getRefreshToken($tenantKey)
    {
        ...
        return isset($this->tenantTokens[$tenantKey]['refreshToken']) ? $this->tenantTokens[$tenantKey]['refreshToken'] : null;
    }
    ...
    function SendEmailToList($emailID, $listID, $sendClassficationCustomerKey)
    {
        ...
        $email->props["SendClassification"] = array(
    ...
    function CreateAndStartDataExtensionImport($dataExtensionCustomerKey, $fileName, $overwrite)
    {
        ...
        $import->props["AllowErrors"] = "true";
        $import->props["DestinationObject"] = array(
            "ObjectID" => $dataExtensionCustomerKey
        );
        $import->props["FieldMappingType"] = "InferFromColumnHeadings";
        $import->props["FileSpec"] = $fileName;
        $import->props["FileType"] = "CSV";
...
class ET_Info extends ET_Constructor
{
    ...
    function __construct($authStub, $objType, $extended = false)
    {
        ...
        $describeRequest["ObjectDefinitionRequest"] = array(
            "ObjectType" => $objType
        );
...
class ET_Asset extends ET_CUDSupportRest
{
    ...
    public function upload()
    {
        ...
        $post = array(
            'file_contents' => '@' . $this->attrs['filePath']
        );
...
class ET_DataExtension_Row extends ET_CUDWithUpsertSupport
{
    ...
    public function post()
    {
        ...
        $overrideProps['Properties'] = array(
            "Property" => $fields
        );
...
function restGet($url, $proxy = NULL)
{
    $ch = curl_init();
    $headers = array(
        "User-Agent: " . getSDKVersion()
 ...
 * @return string The response payload from the REST service
 */
function restPatch($url, $content, $proxy = NULL)
{
    $ch = curl_init();
 ...
 * @return string The response payload from the REST service
 */
function restPut($url, $content, $proxy = NULL)
{
    $ch = curl_init();
...
function restDelete($url, $proxy = NULL)
{
    $ch = curl_init();
